...
def generate_questions(category, difficulty, count=5):
    ...
    """
    logger.info(f"Starting question generation - Category: {category}, Difficulty: {difficulty}, Count: {count}")
...
- 3-6 options should be correct answers
- 4-7 options should be incorrect but plausible answers
...
Make sure the JSON is valid and properly formatted. Do not include any text before or after the JSON array."""
    ...
    try:
        ...
        for i, question in enumerate(questions_data):
            logger.debug(f"Question {i+1}: {len(question.get('options', []))} options, "
...
def add_questions_to_database(questions_data, db_path="./smart10.db"):
    ...
    try:
        ...
        for i, question_data in enumerate(questions_data):
            ...
            logger.debug(f"Question {i+1}: Category={category}, Difficulty={difficulty}, Options={len(options)}")
            ...
            for j, option in enumerate(options):
                if 'text' not in option or 'isCorrect' not in option:
                    logger.warning(f"Question {i+1}, option {j+1} missing required fields")
                    continue
            ...
            logger.debug(f"Question {i+1}: Added {len(options)} options, {correct_count} correct")
            added_count += 1
...
def main():
    ...
    if len(sys.argv) < 3:
        ...
        print("Categories: Technology, Science, Geography, History, Literature, Sports, Music, Art, Film")
    ...
    logger.info(f"Parsed arguments - Category: {category}, Difficulty: {difficulty}, Count: {count}")
    
    # Validate inputs
    valid_categories = ["Technology", "Science", "Geography", "History", "Literature", "Sports", "Music", "Art", "Film"]
    ...
    if difficulty not in valid_difficulties:
        ...
        print(f"Invalid difficulty. Valid difficulties: {', '.join(valid_difficulties)}")
        sys.exit(1)
